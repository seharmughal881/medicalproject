...
export default function Profile() {
    ...
    const fetchProfile = async () => {
      try {
        ...
        if (response.data.data) {
          ...
          setProfileData(response.data as unknown as ProfileData);
        }
        setLoading(false);
      } catch (err: unknown) {
  ...
  const handleSave = async () => {
    try {
      ...
      if (response.data.success && response.data.updateUser) {
        setProfileData(response.data.updateUser);
        setIsEditing(false);
        alert("Profile updated successfully!");
      }
      ...
      alert(axiosError.response?.data?.message || "Error updating profile");
    }
  };
